 ...
 *
 * @tparam L
 *   the `HList` type to select an element from
 * @tparam T
 *   the type to select an element by; subtypes are eligible candidates
 */
...
object SelectCovariant {
  ...
  implicit def recurse[H, L <: HList, U](
      implicit st: SelectCovariant[L, U]): SelectCovariant[H :: L, U] =
